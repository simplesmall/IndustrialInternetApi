...
func CreateMirror(c *gin.Context)(mirror model.Mirror,err error){
	_ = c.BindJSON(&mirror)
	if err = config.DB.Model(&mirror).Create(&mirror).Error;err!=nil{
		return model.Mirror{}, err
	}
	return
}
...
func GetAllApplications(c *gin.Context) (applicationComposer paginate.ApplicationComposer,err error) {
	...
	SQL:=config.DB.Model(&model.Application{})
...
func CreateApplication(c *gin.Context)(application model.Application,err error){
	...
	if err = config.DB.Model(&model.Application{}).Create(&application).Error;err!=nil{
		return model.Application{}, err
	}
	return
}
