...
func GetAllUsers(c *gin.Context) (Composer paginate.UserComposer, err error) {
	...
	SQL:=config.DB.Model(&model.User{}).Where("type = ?",1).Preload("Role").Preload("Role.Permission")
...
func CreateUser(c *gin.Context) (user model.User, affe int64) {
	...
	notfound:=config.DB.Model(&model.User{}).Where("account = ?",userIdList.User.Account).RecordNotFound()
	if !notfound {
		return model.User{},0
	}
...
func CreateTenant(c *gin.Context) (user model.User, affe int64) {
	...
	//给定默认权限
	userIdList.User.Role = roles
	userIdList.User.Type = "2"
	affe = config.DB.Create(&userIdList.User).RowsAffected
	return userIdList.User,affe
}
func GetAllTenants(c *gin.Context) (Composer paginate.UserComposer, err error) {
	page := c.Param("page")
	pagesize := c.Param("pageSize")

	var userList []model.User
	SQL:=config.DB.Model(&model.User{}).Preload("Role").Where("type = ?",2).Preload("Role.Permission")
	Composer,err = paginate.UserPaginator(SQL,page,pagesize,userList)
	return Composer,err
}
...
func CreateEnterUser(c *gin.Context) (user model.User, affe int64) {
	...
	// 将列表关系数据绑定到新建用户上
	userIdList.User.Role = roles
	userIdList.User.Type = "3"
	affe = config.DB.Create(&userIdList.User).RowsAffected
	return userIdList.User,affe
}

func GetAllEnterUsers(c *gin.Context) (Composer paginate.UserComposer, err error) {
	page := c.Param("page")
	pagesize := c.Param("pageSize")

	var userList []model.User
	SQL:=config.DB.Model(&model.User{}).Preload("Role").Where("type = ?",3).Preload("Role.Permission")
	Composer,err = paginate.UserPaginator(SQL,page,pagesize,userList)
	return Composer,err
}